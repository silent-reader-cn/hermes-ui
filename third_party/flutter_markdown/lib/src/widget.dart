...
class _MarkdownWidgetState extends State<MarkdownWidget>
  ...
  void _parseMarkdown() {
    ...
    // PATCH(hermes-ui): degrade to plain text on any builder crash instead
    // of surfacing a widgets-library exception. Builders are third-party
    // extensible and may throw on unusual AST shapes.
    try {
      ...
      FlutterError.reportError(FlutterErrorDetails(
        exception: error,
        stack: stackTrace,
        library: 'markdown widget',
        context: ErrorDescription('while building markdown widgets'),
        informationCollector:
            () => <DiagnosticsNode>[
              ErrorHint(
                'Markdown build failed; falling back to plain text. '
                'Source length: ',
              ),
            ],
      ));
