...
// PATCH(hermes-ui): per-build copy; see PATCH_NOTES.md section 3.
final List<String> _kDefaultBlockTags = <String>[
  'p',
  'h1',
  'h2',
  'h3',
...
// PATCH(hermes-ui): block-tag lookup moved onto the builder instance so
// builder registrations from one MarkdownBody cannot leak into another.
bool _isBlockTagOn(List<String> blockTags, String? tag) =>
...
class MarkdownBuilder implements md.NodeVisitor {
  // PATCH(hermes-ui): per-build block tags (defaults + registered builders).
  final List<String> _blockTags = List<String>.of(_kDefaultBlockTags);
  ...
  List<Widget> build(List<md.Node> nodes) {
    ...
    // PATCH(hermes-ui): rebuild the per-build block tag list instead of
    // mutating the package-global (leaked across MarkdownBody instances).
    _blockTags
      ..clear()
      ..addAll(_kDefaultBlockTags);
  ...
  bool visitElementBefore(md.Element element) {
    ...
    int? start;
    if (_isBlockTagOn(_blockTags, tag)) {
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTagOn(_blockTags, tag)) {
      ...
      // PATCH(hermes-ui): guard against an empty inline stack. After the
      // merging flush above, a link wrapper (`a`) may unwind when its own
      // inline element has already been consumed; popping then would crash.
      if (_inlines.isEmpty) {
  ...
  void _addAnonymousBlockIfNeeded() {
    ...
    if (_isBlockTagOn(_blockTags, _currentBlockTag)) {
    ...
    // PATCH(hermes-ui): merge ALL accumulated inline elements instead of
    // asserting exactly one (`_inlines.single` threw "Bad state: Too many
    // elements" when a link-wrapped block-level image left multiple
    // inline elements on the stack, e.g. [![badge](src)](href)).
    final List<Widget> inlineWidgets = <Widget>[
      for (final _InlineElement inline in _inlines) ...inline.children,
    ];
    ...
    // PATCH(hermes-ui): always drop residual inline shells. When every
    // stacked inline is empty (e.g. p > a > img with a block-level img
    // builder consumed the content), the merged list above is empty and
    // the old code skipped the clear, leaving shells on the stack that
    // tripped the `_inlines.isEmpty` assert at the end of build().
    _inlines.clear();
  }
