...
///
/// 包含 6 个功能入口的显隐状态：任务、看板、工作区、技能、统计、记忆。
class SessionEntryVisibility {
  /// 创建会话列表入口显隐状态，缺省 tasks/skills/insights/workspaces/memory 开、kanban 关。
  ...
  bool get showsAny =>
      tasks || kanban || skills || insights || workspaces || memory;

  /// 检查指定标识的功能入口是否可见。
  ///
  /// 支持的标识：`tasks`, `kanban`, `skills`, `insights`, `workspaces`, `memory`。未知标识默认返回 true。
  ...
  @override
  int get hashCode =>
      Object.hash(tasks, kanban, skills, insights, workspaces, memory);

  @override
  String toString() =>
      'SessionEntryVisibility(tasks: $tasks, kanban: $kanban, skills: $skills, insights: $insights, workspaces: $workspaces, memory: $memory)';
}
