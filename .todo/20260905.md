...
### #64 [P1→已收口] 回合过程折叠胶囊改版：方案 E「时间轴轨」

**主人拍板（2026-09-05）**：#59 蓝竖条改版「有点丑」，要求出多套视觉方案比较；sketches/capsule-variants.html 五案对比后选定 E。

**交付（e6738f8）**：胶囊左缘 1px 发丝轨线（浅 12% 黑/深 15% 白，CupertinoDynamicColor 显式双态）串起整个回合过程——header 行首空心蓝节点（7px 圆环，statusBlueText）+「过程 · 执行代码×8 …」摘要（formatProcessCapsuleSummary 新增 processPrefix 参数，宽度不足前缀优先、明细退省略号）；展开体挂轨延续（Stack+Positioned 画线防无界高度炸）+ 3px 实心节点。与工具卡灰盒天然不同视觉语言。key `collapsible-process-capsule`/`process-capsule-header` 不变。

**验收**：新单测 `process_capsule_prefix_test.dart` 4 例；turn_collapse 8 例 + contrast_scan 零告警；全量 2413 全绿 + analyze 零告警 + 金照 --update-goldens 通过。待主人真机看效果。
