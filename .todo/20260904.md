# Hermes UI TODO — 2026-09-04 归档

> 本轮收口：内置 WebUI Sidecar（S1-S5）、引导页改造（U1-U4）、宽屏双栏铁律、安卓保活假成功修复（K）、多语言切换（L1-L4）。
> 代码全部合入 main（2360 测试全绿 + analyze 零告警 + 独立审查清账 P0-P2 十项全修）。K 线 HyperOS 实机通知落地为后续二次取证项。

---


## [已收口·方向] 内置 WebUI Sidecar — 像 Clash Verge 捆 mihomo 那样一键捆绑 Hermes WebUI 后端（Windows）
...
## [已收口·方向] 引导页改造 — 「内置服务」优先（分段控件+一键启动并连接），并废弃 git clone WebUI 部署流
...
## [已收口·方向] 引导体系宽屏双栏铁律 — 左品牌氛围区 + 右单列表单（第一印象优先）
...
## [已收口·问题] 安卓前台服务保活开关打开后无常驻通知、系统通知类别无对应渠道（原生服务启动失败，app 丢弃插件返回值致假成功日志）
...
## [已收口·方向] 多语言切换设置 — 外观组「语言」三态（自动/中文/English）+ 全仓硬编码中文审计
